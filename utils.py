...
def show_task(task_name: str, solve: Optional[Solver] = None) -> None:
    task_path = os.path.join(base_path, task_name)
    with open(task_path) as f:
        task = json.load(f)

    examples = task["train"]
    _, axs = plt.subplots(len(examples), 2)
    ...
    if solve is None:
        if not os.path.exists(solution_path):
            ...
            return
        imported_solver: Solver = import_module(f"solutions.{task_name.replace('.json', '')}").solve
        solve = imported_solver
